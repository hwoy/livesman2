	...
	if (argc == 2 || argc == 3 || argc == 4) {
		...
		if (argc == 3 || argc == 4) {
			...
			if (!ischarnum(argv[2])) {
				...
				return 1;
			} else if ((maxlives = atoui_noc(argv[2])) < 1) {
				fprintf(stderr,
					"ERR 1 ARG: %s is must >= 1\n\n",
					argv[2]);
				showhelp(grappath(argv[0]));
				return 1;
			}
		...
		if (argc == 4) {
			...
			if (!ischarnum(argv[3])) {
				...
				showhelp(grappath(argv[0]));
				return 1;
			} else if ((addlivestep = atoui_noc(argv[3])) < 1) {
				fprintf(stderr,
					"ERR 2 ARG: %s is must >= 1\n\n",
					argv[3]);
				showhelp(grappath(argv[0]));
				return 1;
			}
...
/*******************************************************************************************************************/
	...
	while (fgets(buff, BUFFSIZE, gfp)) {
		cutnl(buff, 0);
		i = grapstring(cutcharbe(buff, 0x20), DELIM, GP_NLEN, GP_NCHAR,
			       gr);
