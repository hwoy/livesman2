...
unsigned int	hmf_getndata(assem * asmb, unsigned int gid);	/* return numbers of
								 * data */
hms_data       *hmf_getdata(assem * asmb, unsigned int gid, unsigned int index);	/* return a pointer of
											 * data */

hms_group      *hmf_getgroup(assem * asmb, unsigned int gid);	/* return a pointer of
								 * group */
unsigned int	hmf_getngroup(assem * asmb);	/* return numbers of group */

void		hmf_free  (assem * asmb, memman * mm);	/* free allocated space
							 * of assem and lnklist */
